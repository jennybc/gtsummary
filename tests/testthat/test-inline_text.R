...
test_inline1 <- gastric %>% tbl_summary()
test_inline2 <- gastric %>% tbl_summary(by = "trt")
test_inline2b <- gastric %>% tbl_summary(by = "trt") %>% add_p()
...
test_that("inline_text.tbl_summary: with by", {
  expect_error(
    inline_text(test_inline2, variable = "age", column = "No Radiation"),
    NA
  )
  expect_warning(
    inline_text(test_inline2, variable = "age", column = "No Radiation"),
  ...
  expect_error(
    inline_text(test_inline2, variable = "stage", level = "T1", column = "No Radiation"),
    NA
  )
  expect_warning(
    inline_text(test_inline2, variable = "stage", level = "T1", column = "No Radiation"),
...
test_that("inline_text.tbl_summary: with by -  expect errors", {
  expect_error(
    inline_text(test_inline2, variable = "age", column = "No Radddddiation"),
    "No column selected.*"
  )
  expect_error(
    inline_text(test_inline2, variable = "stage", level = "Tsdfgsdfg1", column = "No Radiation"),
    "Is the variable level spelled correctly.*"
  )

  expect_error(
    inline_text(test_inline2, variable = "st55age", level = "T1", column = "No Radiation"),
...
test_inline3 <- lm(albumin ~ age + stage, gastric) %>% tbl_regression()
test_inline4 <- glm(response ~ trt + age + stage, gastric, family = binomial) %>%
...
test_inline_surv_nostrata <-
  survfit(Surv(ttdeath, death) ~ 1, gastric) %>%
  tbl_survival(
    times = c(12, 24),
    time_label = "{time} Months"
  )
...
test_that("inline_text.tbl_survival - with strata", {
  expect_error(
    inline_text(test_inline_surv_strata, strata = "Radiation", time = 24),
    NA
  )
  expect_error(
    inline_text(test_inline_surv_strata2, strata = "Radiation", prob = 0.2),
    NA
  )
  expect_message(
    inline_text(test_inline_surv_strata, strata = "Radiation", time = 30),
    "Specified 'time'*"
  )
  expect_error(
    inline_text(test_inline_surv_strata, strata = "Radiation", time = -2),
  ...
  expect_error(
    inline_text(test_inline_surv_strata, strata = "Radddddddiation", time = 24),
...
test_that("inline_text.tbl_survival - no strata", {
  ...
  expect_warning(
    inline_text(test_inline_surv_nostrata, strata = "Radiation", time = 24),
    "Ignoring strata =*"
  )
})
