...
We'll be using the `gastric` data set throughout this example.  This set contains data from `r nrow(gastric)` gastric cancer patients who received chemotherapy only or chemotherapy and radiation.  The outcome is a binary tumor response.  Each variable in the data frame has been assigned an attribute label (e.g. `attr(gastric$trt, "label") = "Treatment Recieved")`. These labels are displayed in the output table by default.  A data frame without labels will print variable names.

    trt      Treatment Recieved
    age      Age, yrs
...
# printing gastric data
head(gastric) %>% gt()
```

The default output from `tbl_summary()` is meant to be publication ready.  Let's start by creating a table of summary statistics from the `gastric` data set.  The `tbl_summary()` can take, minimally, a data set as the only input, and return descriptive statistics for each column in the data frame.

For brevity, keeping a subset of the variables in the gastric data set.

```{r, message=FALSE}
gastric2 =
  gastric %>%
  select(trt, albumin, stage)

tbl_summary(gastric2)
```

This is a great table, but for the data the summary statistics should be split by treatment group.  To compare two or more groups, include `add_p()` with the function call.

```{r}
tbl_summary(gastric2, by = "trt") %>% add_p()
```
...
- Round the albumin mean and SD to 1 and 2 places, respectively
...
```{r}
tab1 = tbl_summary(gastric2, by = "trt")
tab1
```

To report the median (IQR) of the albumin levels in each group, use the following commands inline.

> The median (IQR) albumin level in the radiation and no radiation groups are `` `r
inline_text(tab1, variable = "albumin", column = "Radiation")` `` and `` `r
inline_text(tab1, variable = "albumin", column = "No Radiation")` ``, respectively.

Here's how the line will appear in your report.

> The median (IQR) albumin level in the radiation and no radiation groups are `r inline_text(tab1, variable = "albumin", column = "Radiation")` and `r inline_text(tab1, variable = "albumin", column = "No Radiation")`, respectively.

If you display a statistic from a categorical variable, include the `level` argument.  

> `` `r
inline_text(tab1, variable = "stage", level = "T1", column = "Radiation")` `` resolves to "`r inline_text(tab1, variable = "stage", level = "T1", column = "Radiation")`"
...
```{r remedy001}
tbl_summary(gastric2, by = "trt") %>%
  as_gt(omit = "footnote_stat_label") %>%
  tab_spanner(label = "Treatment Group",
